...
| D-115 | 2026-09-23 | **The "Demo data" banner (I11) was removed from `banners()`** (`apps/web/lib/status.ts`) at the owner's explicit request, after being told this directly conflicts with I11 ("The UI shows a persistent 'DEMO DATA' banner when active") and that `docs/DEMO.md` requires `DEMO_MODE=true` for the actual hackathon demo. `.env.local`'s `DEMO_MODE` was also set to `false` for normal local use | The owner asked twice, after the tradeoff (mocked data with no on-screen disclosure) was stated plainly and an alternative (a smaller badge) was offered and declined. `banners()` still accepts `demoMode` on its input type rather than dropping it, and `status.test.ts` documents the new behaviour by name, so restoring the banner (e.g. before the live demo, where `DEMO_MODE` must be `true` per `docs/DEMO.md`) is a one-line revert, not a rediscovery | Shrinking it to a small badge (offered, declined); leaving it in place and only hiding it via `DEMO_MODE=false` (declined — owner wanted the banner code gone, not just dormant) |
| D-116 | 2026-09-24 | **`useSigner`'s D-5 "plain EOA" blocker now only runs on the spend-permission grant screen** (`apps/web/lib/useSigner.ts`), opt-in via a new `spendPermission?: { treasuryAddress }` argument instead of the old bare `treasuryAddress?`. `SpendLimitSign.tsx` (the only caller that actually needs a smart-wallet-capable signer) opts in; `PolicySign.tsx`, `AddRecipientSign.tsx`, `ApprovalSign.tsx`, `FreezeFlow.tsx` and `UnfreezeSlot.tsx` call `useSigner()` bare and no longer see the "this wallet cannot give Steward a spending limit" blocker | Found live: a brand-new plain-EOA MetaMask owner was blocked from signing a policy, even though policy activation is a plain EIP-191 message signature checked against the owner's own address server-side (`policy/activate`), not a spend-permission grant — the D-5 restriction (a plain EOA cannot grant a Coinbase Spend Permission) only actually applies to `SpendLimitSign`'s on-chain grant. `PolicySign` previously called `useSigner()` with no treasury argument at all, so even a correctly-derived companion smart wallet (Phase 7 addendum) could never clear the blocker there. Confirmed per-route: recipient, approval and freeze/unfreeze signatures are all checked the same owner-address way, and freeze/unfreeze must never block a plain EOA regardless (I7) | Threading `wallet.treasuryAddress` into every `useSigner` call site so the existing "has code, or is the Smart Wallet connector, or matches a companion" check could still pass (rejected: those five screens sign as the owner, not the treasury, so the check was the wrong one to apply there at all, not merely mis-wired) |
| D-117 | 2026-09-24 | **Revoking a spend permission now sends the on-chain call from the treasury itself, not always from the connected wallet.** New `useTreasuryTransaction()` hook (`apps/web/lib/useTypedDataSigner.ts`) shares the existing companion-derivation helper: when `revokeStep()`'s new `account` field (`apps/web/lib/ownerPath.ts`) equals the connected address it sends a plain `sendTransactionAsync({to, data})` exactly as before; when the treasury is a derived companion smart wallet, it deploys the companion first if needed (through the connected EOA, one of its owners) and then sends `execute(manager, 0, revokeData)` to the companion instead. `FreezeFlow.tsx`'s revoke calls this instead of `sendTransactionAsync` directly | Found live: a companion-treasury owner's "Revoke spending permission" button did nothing usable. Replayed the exact revoke against live Base Sepolia for the reproducing wallet: sent from the connected EOA it reverts with `InvalidSender(eoa, companion)` — `SpendPermissionManager.revoke()` requires `msg.sender` to be the permission's own granting account, which is the companion, not the EOA that owns it. A Coinbase Smart Wallet lets any of its owners call its own `execute()` directly (no bundler/paymaster needed), so the EOA can drive the companion through `execute()` once it exists; simulated end to end on live Base Sepolia (deploy + execute) and confirmed `isRevoked` becomes true, and confirmed the same call from a non-owner address is refused with `Unauthorized()`. Checked "sweep home" for the identical bug: it doesn't have it — sweep is a plain owner text-message signature checked against the owner address, and the actual fund movement runs server-side (`sweepHome` in `@steward/wallet`), nothing is sent from the treasury client-side | A relayer/bundler-based UserOperation for the revoke (rejected: unnecessary — a Smart Wallet owner can call `execute()` on it directly with a plain transaction, no ERC-4337 infra needed for this one call) |
