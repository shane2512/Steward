'use client';
// Installed from the pasted 21st.dev-style spec, faithfully — structure, behaviour and class names
// unchanged (bg-card/border-border/text-foreground/text-muted-foreground/bg-muted all resolve
// through the shadcn token bridge in globals.css, so this renders in our real palette without
// edits). The only file-level change from the original paste: explicit `aria-hidden="true"` instead
// of the bare boolean shorthand, to match this repo's lint config.
...
export interface ProgressMetricCardProps {
  ...
  defaultIndex?: number;
  size?: CardSize;
  /** Show the secondary peak/low/avg stats in the footer. */
  showStats?: boolean;
  /** Value formatting. Defaults to compact in the headline, exact in the tooltip. */
...
// Share of the card (from the right) the chart occupies.
const REGION_W = 62; // %
...
export default function ProgressMetricCard({
  ...
  const chartSeries: ChartSeries[] = visibleSeries.map((s, i) => ({
    ...
    color: s.accent
      ...
      : isMulti
        ? (SERIES_COLORS[i % SERIES_COLORS.length] ?? color.stroke)
        : color.stroke,
  }));
  ...
  return (
    <div className={shell}>
      {/* Chart zone (right-hand side, behind the content) */}
