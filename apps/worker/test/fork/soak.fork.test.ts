...
import { schema, verifyChain, type Db } from '@steward/db';
...
async function seedWallet(i: number): Promise<SoakWallet> {
  ...
  const [user] = await db
    .insert(schema.users)
    .values({ ownerAddress: getAddress(`0x${(0xbeef0000n + BigInt(i)).toString(16).padStart(40, '0')}`) })
    .returning();
...
describe.skipIf(!FORK)(`8.9 soak: ${WALLETS} wallets x ${ROUNDS} rounds`, () => {
  ...
  it(
    'runs the soak: no duplicate execution, and the audit chain still verifies',
    async () => {
      ...
      for (let round = 0; round < ROUNDS; round++) {
        ...
        const jobs = wallets.flatMap((w) =>
          Array.from({ length: TRIGGERS_PER_ROUND }, () =>
            withWalletLock(pool, w.id, async () => {
      ...
      // eslint-disable-next-line no-console -- this IS the deliverable of 8.9.
  ...
  it(
    'I10 under real concurrency: three UNLOCKED iterations for one wallet send once',
    async () => {
      ...
      const outcomes = await Promise.all(
        Array.from({ length: 3 }, () => runIteration(loopDeps(w, () => at), w.id, 'schedule')),
      );
